...
export default {
  ...
  checkScrollPosRelToAnchors() {
    ...
    else for (let i = 0; i < this.anchors.length; i++) {
      // If scrolled to or past the anchor position set this as new furthest and move onto next anchor
      if (window.pageYOffset >= this.anchors[i].offsetTop) furthest = i
      else break
    }
  ...
  setActiveNavTab( tabToActiveIndex) {
    ...
    if (!this.nav.querySelectorAll('li')[tabToActiveIndex].classList.contains(this.activeTabClassName)) {
      // Remove active state from previously active tab
      if (this.nav.querySelectorAll(`.${this.activeTabClassName}`).length) {
        this.nav.querySelectorAll(`.${this.activeTabClassName}`)[0].classList.remove(this.activeTabClassName)
      }

      // Add active state to new active tab
      this.nav.querySelectorAll('li')[tabToActiveIndex].classList.add(this.activeTabClassName)
    }
  }
};
